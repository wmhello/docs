...
const genServiceWorker = async (options, context) => {
    ...
    await workbox_build_1.generateSW(Object.assign({ swDest, globDirectory: context.outDir, cacheId: context.siteConfig.name || "mr-hope", globPatterns,
        ...
        if (warnings.length)
            console.log(chalk_1.blue("PWA:"), chalk_1.black.bgYellow("Warning"), `${warnings.map((warning) => `  · ${warning}`).join("\n")}`);
        if (size > 104857600)
